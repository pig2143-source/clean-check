...
export default function App(){
  ...
  async function retryAI(submissionId){
    ...
    }catch(e){flash(e.message||'AI 重新分析失敗','error');return {ok:false,body:{error:e.message||'AI 重新分析失敗'}}}finally{setLoading(false)}
  }
  ...
  return <div className="page">
    {page==='task' ? <TaskPage task={selected} submission={selected?.submission} onBack={()=>setPage('home')} onSubmit={submitTask} loading={loading}/> : <>
      ...
      <main className="content">
        ...
        {page==='history'&&<HistoryPage submissions={submissions} signedUrl={signedUrl} retryAI={retryAI}/>} 
...
function HistoryPage({submissions,signedUrl,retryAI}){const [filter,setFilter]=useState('all'),[query,setQuery]=useState(''),[selected,setSelected]=useState(null);const list=useMemo(()=>submissions.filter(s=>(filter==='all'||s.status===filter)&&(!query||s.cleaning_tasks?.name?.includes(query)||s.profiles?.display_name?.includes(query))),[submissions,filter,query]);return <><h2>歷史紀錄</h2><div className="search"><Search size={18}/><input className="input" placeholder="搜尋項目或員工" value={query} onChange={e=>setQuery(e.target.value)}/></div><div className="tabs" style={{marginTop:12}}>{[['all','全部'],['approved','合格'],['review','待審核'],['redo','需重做']].map(([v,t])=><button key={v} className={filter===v?'on':''} onClick={()=>setFilter(v)}>{t}</button>)}</div><div className="card">{list.length===0?<div className="empty">沒有符合的紀錄。</div>:list.map(r=><button className="task clickable" key={r.id} onClick={()=>setSelected(r)} style={{width:'100%',border:0,background:'transparent',textAlign:'left'}}><div className="iconbox"><History size={20}/></div><div style={{flex:1}}><b>{r.cleaning_tasks?.name}</b><div className="muted">{r.profiles?.display_name} · {r.work_date}{r.ai_status==='completed'?` · AI ${r.ai_score??'--'} 分`:''}</div></div><Status status={r.status}/><ChevronRight size={18}/></button>)}</div>{selected&&<HistoryModal item={submissions.find(s=>s.id===selected.id)||selected} signedUrl={signedUrl} retryAI={retryAI} close={()=>setSelected(null)}/>}</>}
function HistoryModal({item,signedUrl,retryAI,close}){const [url,setUrl]=useState(''),[busy,setBusy]=useState(false),[current,setCurrent]=useState(item);useEffect(()=>{setCurrent(item);signedUrl(item.photo_path).then(setUrl)},[item.id,item.photo_path]);async function runAI(){setBusy(true);setCurrent(x=>({...x,ai_status:'analyzing',ai_error:null}));try{const result=await retryAI(item.id);if(result?.ok&&result.body?.analysis){const a=result.body.analysis;setCurrent(x=>({...x,ai_status:'completed',ai_score:a.score,ai_verdict:a.verdict,ai_oil_stain:a.oil_stain,ai_water_stain:a.water_stain,ai_trash:a.trash,ai_summary:a.summary,ai_suggestions:a.suggestions,ai_image_quality:a.image_quality,ai_error:null}))}else setCurrent(x=>({...x,ai_status:'failed',ai_error:result?.body?.error||'AI 分析失敗'}))}finally{setBusy(false)}}return <div className="modal" onClick={close}><div className="sheet" onClick={e=>e.stopPropagation()}><div className="row space"><div><h2 style={{margin:0}}>{current.cleaning_tasks?.name}</h2><div className="muted">{current.profiles?.display_name} · {current.work_date}</div></div><button className="button ghost" onClick={close}><X size={18}/></button></div><div className="divider"/>{url?<img className="photo photoTall" src={url} alt="清潔紀錄"/>:<div className="upload">照片載入中…</div>}<p><b>員工備註：</b>{current.note||'無'}</p>{current.manager_note&&<p><b>主管意見：</b>{current.manager_note}</p>}<AIAnalysis submission={current}/>{(!current.ai_status||['failed','pending'].includes(current.ai_status))&&<button className="button ghost" style={{width:'100%',marginTop:10}} disabled={busy} onClick={runAI}><RefreshCw size={16} className={busy?'spin':''}/>{busy?'AI 分析中…':'執行 AI 照片分析'}</button>}<div className="row space" style={{marginTop:12}}><Status status={current.status}/><span className="muted">{formatDateTime(current.created_at)}</span></div></div></div>}
