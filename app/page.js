...
export default function App(){
  ...
  async function submitTask({file,note}){
    try{
      ...
      let submissionId
      if(existing){
        const {data,error}=await supabase.from('cleaning_submissions').update({photo_path:path,note,status:'review',manager_note:'',reviewed_by:null,reviewed_at:null,created_at:new Date().toISOString()}).eq('id',existing.id).select('id').single()
        if(error) throw error
        submissionId=data.id
      }else{
        const {data,error}=await supabase.from('cleaning_submissions').insert({task_id:selected.id,staff_id:session.user.id,photo_path:path,note,work_date:today(),status:'review'}).select('id').single()
        if(error) throw error
        submissionId=data.id
      }
  ...
  return <div className="page">
    {page==='task' ? <TaskPage task={selected} submission={selected?.submission} onBack={()=>setPage('home')} onSubmit={submitTask} loading={loading}/> : <>
      ...
      <main className="content">
        ...
        {page==='history'&&<HistoryPage submissions={submissions} signedUrl={signedUrl} canReview={canManage} review={review}/>} 
...
function Login({notice}){
  ...
  return <div className="login dpLogin"><div className="loginAura auraOne"/><div className="loginAura auraTwo"/><form className="loginbox brandLoginCard" onSubmit={go}>
    ...
    <div className="loginFooter">DP Clean Enterprise <b>v6.2</b></div>
  </form></div>
}
...
function HistoryPage({submissions,signedUrl,canReview=false,review}){const [filter,setFilter]=useState('all'),[query,setQuery]=useState(''),[selected,setSelected]=useState(null);const list=useMemo(()=>submissions.filter(s=>(filter==='all'||s.status===filter)&&(!query||s.cleaning_tasks?.name?.includes(query)||s.profiles?.display_name?.includes(query))),[submissions,filter,query]);return <><h2>歷史紀錄</h2><div className="search"><Search size={18}/><input className="input" placeholder="搜尋項目或員工" value={query} onChange={e=>setQuery(e.target.value)}/></div><div className="tabs" style={{marginTop:12}}>{[['all','全部'],['approved','合格'],['review','待審核'],['redo','需重做']].map(([v,t])=><button key={v} className={filter===v?'on':''} onClick={()=>setFilter(v)}>{t}</button>)}</div><div className="card">{list.length===0?<div className="empty">沒有符合的紀錄。</div>:list.map(r=><button className="task clickable" key={r.id} onClick={()=>setSelected(r)} style={{width:'100%',border:0,background:'transparent',textAlign:'left'}}><div className="iconbox"><History size={20}/></div><div style={{flex:1}}><b>{r.cleaning_tasks?.name}</b><div className="muted">{r.profiles?.display_name} · {r.work_date}</div></div><Status status={r.status}/><ChevronRight size={18}/></button>)}</div>{selected&&<HistoryModal item={submissions.find(s=>s.id===selected.id)||selected} signedUrl={signedUrl} canReview={canReview} review={review} close={()=>setSelected(null)}/>}</>}
function HistoryModal({item,signedUrl,canReview,review,close}){const [url,setUrl]=useState(''),[rejecting,setRejecting]=useState(false),[reason,setReason]=useState(''),[busy,setBusy]=useState(false);useEffect(()=>{signedUrl(item.photo_path).then(setUrl)},[item.id,item.photo_path]);async function submitReview(status,note=''){if(!review||busy)return;setBusy(true);await review(item.id,status,note);setBusy(false);close()}return <div className="modal" onClick={close}><div className="sheet" onClick={e=>e.stopPropagation()}><div className="row space"><div><h2 style={{margin:0}}>{item.cleaning_tasks?.name}</h2><div className="muted">{item.profiles?.display_name} · {item.work_date}</div></div><button className="button ghost" onClick={close}><X size={18}/></button></div><div className="divider"/>{url?<img className="photo photoTall" src={url} alt="清潔紀錄"/>:<div className="upload">照片載入中…</div>}<p><b>員工備註：</b>{item.note||'無'}</p>{item.manager_note&&<p><b>主管意見：</b>{item.manager_note}</p>}<div className="row space" style={{marginTop:12}}><Status status={item.status}/><span className="muted">{formatDateTime(item.created_at)}</span></div>{canReview&&item.status==='review'&&<div className="reviewActions" style={{marginTop:18}}><div className="divider"/><h3 style={{margin:'0 0 12px'}}>主管審核</h3>{rejecting?<><label className="label">退回原因</label><textarea className="input textarea" value={reason} onChange={e=>setReason(e.target.value)} placeholder="請填寫需要重新清潔或補拍的原因"/><div className="row" style={{marginTop:10}}><button className="button ghost" style={{flex:1}} disabled={busy} onClick={()=>{setRejecting(false);setReason('')}}>取消</button><button className="button danger" style={{flex:1}} disabled={busy||!reason.trim()} onClick={()=>submitReview('redo',reason.trim())}>{busy?'處理中…':'確認退回'}</button></div></>:<div className="row"><button className="button primary" style={{flex:1}} disabled={busy} onClick={()=>submitReview('approved','')}><CheckCircle2 size={17}/>{busy?'處理中…':'通過'}</button><button className="button danger" style={{flex:1}} disabled={busy} onClick={()=>setRejecting(true)}><XCircle size={17}/>退回重拍</button></div>}</div>}</div></div>}
