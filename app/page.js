...
import { Camera, CheckCircle2, ChevronRight, ClipboardCheck, History, Home, Image as ImageIcon, ListChecks, LogOut, Plus, RefreshCw, Search, Settings, Sparkles, Store, Trash2, UserRound, Users, X, XCircle } from 'lucide-react'
  ...
  return <div className="page">
    {page==='task' ? <TaskPage task={selected} submission={selected?.submission} onBack={()=>setPage('home')} onSubmit={submitTask} loading={loading}/> : <>
      ...
      <main className="content">
        ...
        {page==='manage'&&canManage&&<ManagePage tasks={tasks} profiles={profiles} saveTask={saveTask} deleteTask={deleteTask}/>} 
...
function ManagePage({tasks,profiles,saveTask,deleteTask}){const [editing,setEditing]=useState(null),[showForm,setShowForm]=useState(false);return <><div className="row space"><div><h2 style={{marginBottom:2}}>管理設定</h2><div className="muted">主管專用</div></div><button className="button primary" onClick={()=>{setEditing(null);setShowForm(true)}}><Plus size={18}/>新增</button></div><div className="sectionTitle"><h3 style={{margin:0}}>清潔項目</h3><span className="muted">{tasks.filter(t=>t.active).length} 個啟用</span></div><div className="card">{tasks.map(t=><div className="task" key={t.id}><div className="iconbox"><ListChecks size={20}/></div><div style={{flex:1}}><b>{t.name}</b><div className="muted">{t.area} · {t.schedule_label} · {t.active?'啟用':'停用'}</div></div><button className="button ghost" onClick={()=>{setEditing(t);setShowForm(true)}}>編輯</button><button className="button danger" onClick={()=>deleteTask(t.id)}><Trash2 size={16}/></button></div>)}</div><div className="sectionTitle"><h3 style={{margin:0}}>帳號名單</h3></div><div className="card">{profiles.map(p=><div className="task" key={p.id}><div className="iconbox"><UserRound size={20}/></div><div style={{flex:1}}><b>{p.display_name}</b><div className="muted">{p.role==='manager'?'主管':'員工'}</div></div><span className="pill">{p.role}</span></div>)}</div>{showForm&&<TaskForm task={editing} close={()=>setShowForm(false)} save={async(v)=>{await saveTask(v,editing?.id);setShowForm(false)}}/>}</>}
